	...
	ImGui::Columns(2, nullptr, false);
	{
		ImGui::BeginChild(XorStr("legitbot.aimbot"), ImVec2(), true, ImGuiWindowFlags_MenuBar);
		{
			...
			if(C::Get<bool>(Vars.bAimRecord))
				ImGui::Combo(XorStr("Record Type##legithbot"), &C::Get<int>(Vars.bAim_Rage_Lagcompensation_Type), XorStr("Best\0Best & Newest\0All\0\0"));
		...
		ImGui::BeginChild(XorStr("visuals.esp"), ImVec2(0, 0), true, ImGuiWindowFlags_MenuBar);
		{
				...
				CTab{ "chams", [&style]()
			{
				...
				if (C::Get<bool>(Vars.bEspChamsEnemies) || C::Get<bool>(Vars.bEspChamsAllies))
				{
					...
					ImGui::Checkbox(XorStr("Record"), &C::Get<bool>(Vars.bEspChamsRecord));
